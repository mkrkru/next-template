...
export function transform(
  ...
  if (init.length !== res.length)
    console.error("init.length must be equal to res.length");
  ...
  for (let i = 0; i < init.length - 1; i++) {
    const f = (y: number) =>
      (y - init[i]) / ((init[i + 1] - init[i]) / (res[i + 1] - res[i])) +
      res[i];
    values.push({ range: [init[i], init[i + 1]], func: f });
  }

  let x: any | number = values.find(
    (v: any) => value >= v.range[0] && value <= v.range[1],
  );
  if (!x) return res[0];
  else x = x.func(value);
